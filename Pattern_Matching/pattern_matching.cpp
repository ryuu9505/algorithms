...
void BruteForce(string t, string p)
{
    int i = 0, // i : index of text
        j = 0; // j : index of pattern
    while (i < t.length())
    {
        if (t[i] != p[j])
        {
            i = i - j + 1; // last start position + 1
            j = 0;
        }
        else
        {
...
{
    vector<int> next(p.length() + 1, 0); // next[i] : p[0] ~ p[i-1]의 suffix/postfix 최대일치수
    next[0] = -1;
...
{
    vector<int> next = KMP_Preprocess(p);
